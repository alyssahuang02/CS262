...
def test_registration_flow():
    ...
    server.register_user(request, context)
    ...
    assert server.active_accounts["dale"] == "mock ip address"
...
#     # Testing client receive flow
#     server = ChatServicer()

#     # Setting up mocks
#     new_route_guide_pb2.Note=MagicMock()
#     request = MagicMock()
#     request.text = "dale"

#     # Test username does not exist
#     server.unsent_messages["dale"] = ["hi dale 1", "hi dale 2"]
#     server.client_receive_message(request, None)
#     assert len(server.unsent_messages["dale"]) == 0
    # new_route_guide_pb2.Note.assert_called_once()
    # new_route_guide_pb2.Text.assert_called_with(text=USER_DOES_NOT_EXIST)
...
def test_display_accounts():
    # TODO: After figuring out a common regex
    pass